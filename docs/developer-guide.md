...
This project (**image-ai-builder**) is built on the ts-liveview stack (WebSocket, server-rendered JSX, better-sqlite3-proxy). The guide below is adapted from the template; some paths and examples may differ.
...
```bash
# This repo (image-ai-builder)
git clone <repo-url> image-ai-builder
cd image-ai-builder
./scripts/init.sh
npm start
...
cd my-app && ./scripts/init.sh && npm start
```
...
```
image-ai-builder/          # (or your project root)
├── server/
│   ├── app/
...
│   └── migrations/
├── docs/                  # Developer guide, AI agent guidelines
├── client/                # Client bundle source
├── build/                 # Built client bundle
├── public/                # Static assets (css, js, images)
├── scripts/               # Setup & deployment (init.sh, new-route.sh, deploy.sh, ...)
└── package.json
```
...
export function handleToggleTask(data: { taskId: number }, context: Context) {
  let task = find(proxy.task, { id: data.taskId })
...
`emit(url, data)` for actions. For a full CRUD example, see the ts-liveview template docs or existing pages in `server/app/pages/` (e.g. manage-labels, app-project).
...
2. **Explore code**
   - `server/app/pages/home.tsx`, `app-home.tsx`, `manage-labels.tsx`
   - `server/app/routes.tsx` (routeDict and page modules)
   - `db/proxy.ts` (generated from erd.txt)

3. **Build something**
   - New page and route (see “Add a New Page” in Common Tasks)
   - Real-time feature with `emit()` and a resolve handler

4. **Review examples**
   - Study pages in `server/app/pages/`
   - ts-liveview examples at liveviews.cc
...
**Based on**: ts-liveview template + better-sqlite3-proxy; adapted for image-ai-builder. For DB workflow see also `db/README.md`.
...
1. Create `server/app/pages/my-page.tsx` (or use `./scripts/new-route.sh` to scaffold from a template)
2. Export `routes` and add the page to `routeDict` in `server/app/routes.tsx` (import and `...MyPage.routes`)
...
In this project, each page module uses a default export with a `routes` property; add your URL and spread it in `routeDict` in `server/app/routes.tsx`:
...
Then in `server/app/routes.tsx`: `import BlogPosts from './pages/blog-posts.js'` and add `...BlogPosts.routes` to `routeDict`. If you scaffold the page with `./scripts/new-route.sh`, it can insert the import and spread into `routeDict` for you.
...
See existing pages in `server/app/pages/` (e.g. `manage-labels.tsx`, `app-project.tsx`) for CRUD, real-time updates, session, and route wiring. The ts-liveview template also has a Blog Posts Manager example in its docs.
